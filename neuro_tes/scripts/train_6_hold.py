...
class NeuroEvolution:
    ...
    def evaluate(self, individual):
        ...
        Tambahkan penalti untuk terlalu sering HOLD.
        """
        ...
        # --------------------------
        # Tambahan: Hitung frekuensi HOLD
        # --------------------------
        hold_count = 0
        ...
        for t in range(0, len(self.trend) - 1):
            ...
            trade_amount = ((1 + compound_factor) * 0.065) * balance  # 6.5% * (1+0.5) (bisa disesuaikan)
        ...
        # Contoh: fitness = winrate - (hold_ratio * 10)
        # Semakin sering HOLD, semakin besar penalti.
        fitness_value = winrate - (hold_ratio * 10)

        # Pastikan fitness tidak negatif (opsional)
    ...
    def evolve(self, generations=50):
        ...
        for epoch in range(generations):
            ...
            print(f"✅ Epoch {epoch+1} - Winrate+Hold Terbaik: {fittest.fitness:.2f}, Rata-rata: {avg_fitness:.2f}, Terburuk: {worst_fitness:.2f}")
...
print(f"\n🚀 Model terbaik disimpan dengan Fitness (winrate-penaltiHOLD) {best_model.fitness:.2f}")
...
def simulate_trading(model, trend, window_size, initial_money, num_features):
    ...
    for t in range(len(trend) - 1):
        ...
        # (Contoh: paksa agar TIDAK hold pada simulasi - di sini 100%? Silakan atur)
        if np.random.rand() < 1:  # Ubah jika ingin ada HOLD
            action = np.random.choice([1, 2])  # Hanya BUY / SELL di sini
...
# -----------------------------
# 7. Plot Hasil Trading dan Indikator Supply/Demand (Opsional)
# -----------------------------
def plot_trading(df, buy_points, sell_points, supply_zones, demand_zones):
    ...
    plt.title('Hasil Trading dengan Model (Winrate & Penalti HOLD)')
    plt.xlabel('Waktu')

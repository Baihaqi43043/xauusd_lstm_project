...
class NeuroEvolution:
    ...
    def evaluate(self, individual):
        ...
        risk_reward_ratio = 3  # RR = 1:3 (SL = 2%, TP = 6%)
        cooldown_period = 15  # JEDA ANTARA TRADE BARU (misal: 10 candle)
        ...
        for t in range(0, len(self.trend) - 1):
            ...
            trade_amount = random.uniform(0.10, 0.30) * balance  

            # Gunakan ATR untuk menentukan Stop Loss & Take Profit
            atr = np.std(self.trend[max(0, t-30):t])  # ATR dari 30 candle terakhir
            stop_loss = price - atr * 2.2  # SL = 2 x ATR
            take_profit = price + (atr * 2.2 * risk_reward_ratio)  # TP = 6 x ATR

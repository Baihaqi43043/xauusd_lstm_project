...
class NeuroEvolution:
    ...
    def act(self, individual, state):
        ...
        if np.random.rand() < 0.15:
            action = np.random.choice([1, 2])  # Paksa BUY atau SELL sesekali

        return action
    
    def evaluate(self, individual):
        """
        Evaluasi fitness berdasarkan winrate (persentase kemenangan).
        Nilai profit masih dihitung/di-logging, namun TIDAK memengaruhi fitness utama.
        """
        ...
        for t in range(0, len(self.trend) - 1):
            ...
            trade_amount = ((1 + compound_factor) * 0.065) * balance  # 6.5% * (1 + 0.5) = 18.75%? (disesuaikan)
        ...
        if closed_trades == 0:
            ...
            # (atau bisa juga kita beri penalti besar agar tidak dianggap solusi bagus)
            winrate = 0  
            avg_duration = 0
            max_duration = 0
        else:
        ...
        # -- INI BAGIAN PENTING: FITNESS = WINRATE --
        individual.fitness = winrate
        return winrate


    def evolve(self, generations=50):
        """Proses evolusi dengan memprioritaskan winrate tertinggi."""
        ...
        for epoch in range(generations):
            ...
            # Sort populasi dari fitness terbesar (winrate tertinggi) ke terkecil
            ...
            print(f"✅ Epoch {epoch+1} - Winrate Terbaik: {fittest.fitness:.2f}, Rata-rata: {avg_fitness:.2f}, Terburuk: {worst_fitness:.2f}")
            logging.info(f"Epoch {epoch+1}, Winrate Terbaik: {fittest.fitness:.2f}, Rata-rata: {avg_fitness:.2f}, Terburuk: {worst_fitness:.2f}")

            # Jika fitness drop drastis, kurangi mutasi (opsional, bisa dihapus jika tidak diperlukan)
            if epoch > 1 and previous_fitness is not None:
                if fittest.fitness < previous_fitness * 0.5:
                    print("⚠️ Winrate turun drastis, mengurangi mutasi!")
                    logging.info("⚠️ Winrate turun drastis, mengurangi mutasi!")
            ...
            # Crossover + Mutasi untuk membentuk populasi baru
...
print(f"\n🚀 Model terbaik disimpan dengan WINRATE {best_model.fitness:.2f}%")
logging.info(f"Model terbaik disimpan dengan WINRATE {best_model.fitness:.2f}%")
...
def simulate_trading(model, trend, window_size, initial_money, num_features):
    ...
    for t in range(len(trend) - 1):
        ...
        trade_amount = ((1 + compound_factor) * 0.065) * balance
        stop_loss = price - atr * 3.5
        take_profit = price + (atr * 3.5 * risk_reward_ratio)
        ...
        # Tutup posisi jika mencapai SL/TP atau durasi maksimum
        for pos in inventory[:]:
            open_price = pos['open_price']
            invested_amount = pos['invested_amount']
            position_type = pos['position_type']
            ...
            if close_position:
                ...
                balance += invested_amount + profit_trade
                inventory.remove(pos)
                # Tandai titik close (opsional)
                if position_type == "BUY" and profit_trade > 0:
                    sell_points.append(t)
                elif position_type == "SELL" and profit_trade > 0:
                    buy_points.append(t)
...
# Simulasikan trading dengan model terbaik (berfokus winrate)
...
def plot_trading(df, buy_points, sell_points, supply_zones, demand_zones):
    ...
    plt.title('Hasil Trading dengan Model (Fokus Winrate)')
    plt.xlabel('Waktu')

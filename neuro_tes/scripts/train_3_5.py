...
data_file = '../data/gold_usd_preprocessed.csv'  # Ganti dengan path CSV baru Anda
...
class NeuroEvolution:
    ...
    def evaluate(self, individual):
        ...
        MAX_BARS_OPEN = 5 * 96    # 3 hari = 288 bar
        ...
        for t in range(0, len(self.trend) - 1):
            ...
            trade_amount = ((1 + compound_factor) * 0.15) * balance  # Menggunakan 1% dari saldo untuk setiap trade

            atr = np.std([self.trend[i][0] for i in range(max(0, t-96), t)])
...
def simulate_trading(model, trend, window_size, initial_money, num_features):
    ...
    for t in range(len(trend) - 1):
        ...
        atr = np.std([trend[i][0] for i in range(max(0, t-96), t)])
        trade_amount = ((1 + compound_factor) * 0.15) * balance  # Menggunakan 1% dari saldo untuk setiap trade
        stop_loss = price - atr * 3.5
        take_profit = price + (atr * 3.5 * risk_reward_ratio)
        trailing_stop = stop_loss  # Initialize trailing stop
